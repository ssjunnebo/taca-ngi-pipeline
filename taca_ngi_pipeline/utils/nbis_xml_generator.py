...
class xml_generator(object):
    ...
    def __init__(
        ...
        try:
            self.pcon = pcon
            assert self.pcon, "Could not connect to {} database in StatusDB".format(
                "project"
            )
            self.fcon = fcon
            assert self.fcon, "Could not connect to {} database in StatusDB".format(
                "flowcell"
            )
            self.xcon = xcon
            assert self.xcon, "Could not connect to {} database in StatusDB".format(
                "x_flowcells"
            )
            self._check_and_load_project(project)
            assert isinstance(self.project, dict), (
                "Could not get proper project document for {} from StatusDB".format(
                    project["project_id"]
                )
            )
            self.samples_delivered = self.project.get("staged_files", {})
            assert self.samples_delivered, (
                "No delivered samples for project {}, cannot generate XML files".format(
                    project["project_id"]
                )
            )
    ...
    def generate_xml_and_manifest(self, return_string_dict=False):
        ...
        for sample_stat in self._collect_sample_stats():
            ...
            experiment_xml_string += (
                '\t<EXPERIMENT alias="{alias}" center_name="">\n'
                "\t\t<TITLE>{title}</TITLE>\n"
            ...
            run_xml_string += (
                '\t<RUN alias="{alias}" run_center="National Genomics Infrastructure, Stockholm" center_name="">\n'
        ...
        with (
            open(
                os.path.join(
                    self.outdir, "{}_experiments.xml".format(self.project["project_id"])
                ),
                "w",
            ) as exml,
            open(
                os.path.join(
                    self.outdir, "{}_runs.xml".format(self.project["project_id"])
                ),
                "w",
            ) as rxml,
        ):
    ...
    def _generate_manifest_file(self, exp_details):
        ...
        fasta_file_names = list(
            self.samples_delivered[exp_details["discriptor"]].keys()
        )
        ...
        for f in fasta_r1_files:
            ...
            if exp_details["layout"] == "<PAIRED></PAIRED>":
                fcontents += ("FASTQ\t{}\n").format(
                    next(s for s in fasta_r2_files if fname in s)
                )
            with open(
                "{}/{}_manifest.txt".format(manifestdirPath, fname.replace("/", ".")),
                "w",
            ) as manfile:
    ...
    def _stats_from_flowcells(self):
        ...
        for fc, fc_info in six.iteritems(self.flowcells):
            fc_obj = (
                self.xcon.get_entry(fc_info["run_name"])
                if fc_info["db"] == "x_flowcells"
                else self.fcon.get_entry(fc_info["run_name"])
            )
            if not fc_obj:
                self.LOG.warn(
                    "Could not fetch flowcell {} from {} db, will remove it from list".format(
                        fc_info["run_name"], fc_info["db"]
                    )
                )
                continue
            ...
            for lane_stat in (
                ...
                if (
                    lane_sample.startswith(self.project["project_id"])
                    and lane_sample in self.samples_delivered
                    and lane_sample not in fc_info["samples"]
                ):
                    prep_id = "A"
                    ...
                    if not self.ignore_lib_prep:
                        try:
                            ...
                            prep_id_list = [
                                pid
                                for pid, seqruns in six.iteritems(sample_preps_fcs)
                                if full_run_id in seqruns
                            ]
                            assert len(prep_id_list) == 1
                            prep_id = prep_id_list[0]
                        except AssertionError:
                            self.LOG.warning(
                                "Not able to find prep id for sample '{}' - FC '{}'. Found prep seq map is '{}'".format(
                                    lane_sample, full_run_id, sample_preps_fcs
                                )
                            )
                            self.LOG.warning(
                                "Generated XML file may not be reliable, double check the file manually and run with '--ignore-lib-prep' option if neccesary"
                            )
                    prep_inst_key = "{}_{}".format(prep_id, instrument_key)
                    if prep_inst_key not in self.sample_aggregated_stat[lane_sample]:
                        self.sample_aggregated_stat[lane_sample][prep_inst_key] = {
                            "xml_text": fc_info["instrument"],
                            "runs": [],
                        }
                    self.sample_aggregated_stat[lane_sample][prep_inst_key][
                        "runs"
            ...
            if len(fc_info["samples"]) == 0:
                self.LOG.warn(
                    "No sample were found for project {} in fc {}, this is so weird".format(
                        self.project["project_id"], fc_info["run_name"]
                    )
                )
    ...
    def _set_project_design(self):
        ...
        lib_meth_match = re.search(
            r"^(.*?),(.*?),(.*?),(.*?)[\[,](.*)$",
            self.project.get("details", {}).get("library_construction_method", ""),
        )
        if lib_meth_match:
        ...
        if not p_type or "user" in p_type or "house" in p_type:
            dp_design = (
                proj_app or p_input if p_input.lower() != "library" else "Sample"
            )
            dp_protocol = "NA"
        else:
            dp_design = p_type
            dp_protocol = "{}{}".format(
                p_type, ", {}".format(p_option) if p_option else ""
            )
        ...
        if proj_app.lower() == "metagenomics":
            ...
            dp_strategy = (
                "miRNA-Seq"
                if self.project.get("details", {}).get("bioinformatic_qc", "").lower()
                == "mirna-seq"
                else "RNA-Seq"
            )
        ...
        if seq_setup.startswith("1x"):
            ...
            self.LOG.warn(
                "Was not able to fetch sequencing setup from couchdb for project {}, so choosing PAIRED".format(
    ...
    def _check_and_load_flowcells(self, flowcells):
        ...
        if not flowcells or not isinstance(flowcells, dict):
            self.LOG.info(
                "Fetching flowcells sequenced for project '{}' from StatusDB".format(
                    self.project["project_id"]
                )
            )
            flowcells = {}
            flowcells.update(
                self.fcon.get_project_flowcell(
                    self.project["project_id"],
                    self.project.get("open_date", "2015-01-01"),
                )
            )
            flowcells.update(
                self.xcon.get_project_flowcell(
                    self.project["project_id"],
                    self.project.get("open_date", "2015-01-01"),
                )
            )
    ...
    def _check_and_load_lib_preps(self, ignore_lib_prep=False):
        ...
        if not self.ignore_lib_prep:
            for sample in self.samples_delivered.keys():
                sample_preps = (
                    self.project.get("samples", {})
                    .get(sample, {})
                    .get("library_prep", {})
                )
                for prep, prep_info in six.iteritems(sample_preps):
                    self.sample_prep_fc_map[sample][prep] = prep_info.get(
                        "sequenced_fc", []
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="<project id>",
        help="NGI project id for which XML files are generated",
    )
    parser.add_argument(
        "--outdir",
        type=str,
        default=os.getcwd(),
        help="Output directory where the XML files will be saved",
    )
    parser.add_argument(
        "--ignore-lib-prep",
        default=False,
        action="store_true",
        help="Dont take in account the lib preps",
    )
    ...
    xgen = xml_generator(
        ...
        ignore_lib_prep=kwargs["ignore_lib_prep"],
    )
